...
class DotWriter(Writer):
    ...
    def write_node(self, node):
        ...
        # For very large graphs, long labels make layouts ugly. Emit a
        # short label (first line) and put the full human-readable label
        # into the tooltip so users can inspect it in interactive viewers.
        full_label = node.label
        short_label = full_label.split("\\n")[0]
        # Truncate extremely long short labels to keep node boxes compact.
        if len(short_label) > 60:
            short_label = short_label[:57] + "..."

        self.write(
            '%s [label="%s", tooltip="%s", style="filled", fillcolor="%s",'
            ' fontcolor="%s", group="%s", fontsize="10", fontname="DejaVu Sans"];'
            % (
                node.id,
                short_label,
                full_label,
