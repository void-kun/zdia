...
def get_module_name(filename, root: str | None = None):
...
class Scope:
    ...
    def __init__(self, table):
        ...
        name = table.get_name()
        # symtable uses names like '<listcomp>' or '<lambda>' for inner
        # scopes; normalize them to plain identifiers ('listcomp', 'lambda')
        # so that other parts of the analyzer can reference them without
        # angle brackets.
...
class ExecuteInInnerScope:
    ...
    def __enter__(self):
        ...
        if inner_ns not in analyzer.scopes:
            # Symtable may not always create explicit inner scopes for
            # comprehensions/lambdas in all Python versions/configs. To
            # remain robust, create a lightweight synthetic Scope-like
            # object so analysis can proceed. This mirrors the minimal
            # interface used by CallGraphVisitor (defs, name, type).
            analyzer.logger.debug("Creating synthetic inner scope for %s" % inner_ns)
            ...
            sc = _SyntheticScope()
            sc.name = scopename
            sc.type = "comprehension"
            sc.defs = {}
            analyzer.scopes[inner_ns] = sc
