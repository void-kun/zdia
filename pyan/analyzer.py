...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def __init__(self, filenames, root: str | None = None, logger=None):
    ...
    def analyze_functiondef(self, ast_node):
        ...
        for deco in ast_node.decorator_list:
            # Visit the decorator expression to ensure any uses are recorded.
            self.last_value = None
            self.visit(deco)
            deco_node = self.last_value
            if isinstance(deco_node, Node):
                # record both short and fully-qualified names when available
                deco_names.append(deco_node.name)
                try:
                    deco_names.append(deco_node.get_name())
                except Exception:
                    pass

            # If the decorator is a call like @decorator(args...), try to
            # extract the base (decorator.func) so that decorator factories
            # (calls returning decorators) are recognized. Also attempt to
            # resolve simple alias imports (e.g. 'rw' -> functools.wraps).
            base = deco.func if isinstance(deco, ast.Call) else deco
            base_name = None
            try:
                base_name = get_ast_node_name(base)
            except Exception:
                base_name = None

            # If base is a plain name, try to resolve it to a Node (which may
            # represent an imported item under an alias) and record its full name.
            if isinstance(base, ast.Name):
                resolved = self.get_value(base.id)
                if isinstance(resolved, Node):
                    deco_names.append(resolved.name)
                    try:
                        deco_names.append(resolved.get_name())
                    except Exception:
                        pass

            if isinstance(base_name, str):
                deco_names.append(base_name)
            self.last_value = None
