...
class VisualGraph(object):
    ...
    def from_visitor(cls, visitor, options=None, logger=None):
        ...
        group_by_toplevel = options.get("group_by_toplevel", False)
        ...
        for node in visited_nodes:
            ...
            if grouped and group_ns != prev_namespace:
                ...
                id_label = node.get_namespace_label()
                human_label = group_ns if group_ns else node.get_namespace_label()
                subgraph = cls(id_label, human_label)
