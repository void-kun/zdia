...
def create_callgraph(
    ...
    # Improved default GraphViz options to help large-graph layout.
    # These can be overridden by callers by adjusting the options passed
    # into the writers if necessary.
    dot_options = [
        f'rankdir="{rankdir}"',
        'splines="true"',
        'overlap="false"',
    ...
    if format == "dot":
        writer = DotWriter(graph, options=dot_options, output=stream)
        writer.run()

    elif format == "html":
        writer = HTMLWriter(graph, options=dot_options, output=stream)
        writer.run()

    elif format == "svg":
        writer = SVGWriter(graph, options=dot_options, output=stream)
        writer.run()
    else:
